 ...
 * @param queryFn 查询函数
 */
function internalWaitForElement<R>(
  selector: string,
  timeout = 10000,
  queryFn = document.querySelector.bind(document),
): Promise<R> {
...
export async function waitForElement(
  ...
  return internalWaitForElement(selector, timeout)
}

/**
 * querySelectorAll 等待元素加载
 * @param selector 元素选择器
 * @param timeout 超时时间
 */
export async function waitForElements(
  ...
  const nodeList = await internalWaitForElement<NodeListOf<HTMLElement>>(
    selector,
    timeout,
